...
function GetTrackUnderMouseCursor()
...
function ToggleMuteOnTargetTrack(mouse, select, group, exclusive)
    if not mouse then -- mute all selected tracks
        for i = 0, reaper.CountSelectedTracks(0) - 1 do
            ToggleMuteOnTrack(reaper.GetSelectedTrack(0, i), group, i == 0 and exclusive)
        end
    elseif mouse then -- mute track under mouse without changing selection
        local track = GetTrackUnderMouseCursor(0, 0)
...
function ToggleSoloOnTargetTrack(mouse, select, group, exclusive, in_place)
    if not mouse then -- solo all selected tracks
        for i = 0, reaper.CountSelectedTracks(0) - 1 do
...
function CycleTargetTrackRecMonStates(audio_states, instrument_states)
    local track = GetTrackUnderMouseCursor() or reaper.GetSelectedTrack(0, 0)
    if track then
